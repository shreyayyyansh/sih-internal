...
// Water leaks surface away from the pipe — wider radius needed
const RADIUS_M          = 25;
const GEOHASH_PRECISION = 7;  // precision 7 = ~76m tile, must match encode call
...
export const waterCheck = async (req, res) => {
  try {
    ...
    console.log(`[Water] Duplicate found ${closest.distance.toFixed(1)}m away — reportId: ${closest.reportId}`);
    return res.status(200).json({ duplicateFound: true, data: closest });

  } catch (error) {
    console.error('[waterCheck] error:', error);
