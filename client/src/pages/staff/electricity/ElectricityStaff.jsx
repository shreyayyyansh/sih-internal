...
const ZIP_ICON = "https://cdn-icons-png.flaticon.com/512/2983/2983973.png"; // Lighting/Zap icon
const ROUTE_COLOR = "#facc15"; // Yellow for electricity
...
export default function ElectricityStaffDashboard() {
  ...
  useEffect(() => {
    ...
    const updateLocation = (position) => {
      ...
      const staffData = {
        name: user.name || "Grid Technician",
        email: user.email,
  ...
  const handleUploadProof = async (taskId, event) => {
    ...
    try {
      ...
      setVerificationResult({
        success: true,
        title: "Grid Restored!",
        message: reasoning || "The AI confirmed the electrical fault has been fixed.",
      ...
      <div className="fixed inset-0 z-[6000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
        <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
          <div className="flex justify-center mb-4">
            <div className={`p-4 rounded-full ${isSuccess ? 'bg-yellow-100' : 'bg-rose-100'}`}>
              {isSuccess ? <ShieldCheck className="w-10 h-10 text-yellow-600" /> : <AlertOctagon className="w-10 h-10 text-rose-600" />}
            </div>
          </div>
          <h2 className={`text-2xl font-black text-center mb-2 ${isSuccess ? 'text-yellow-800' : 'text-rose-800'}`}>{verificationResult.title}</h2>
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6">
            <p className="text-slate-600 text-sm font-medium text-center leading-relaxed">"{verificationResult.message}"</p>
          </div>
          <button onClick={() => setVerificationResult(null)} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg ${isSuccess ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-slate-900'}`}>
      ...
      <div className="fixed inset-0 z-[5000] bg-white h-[100dvh] w-full flex flex-col relative">
        ...
        <div className="absolute top-0 left-0 right-0 z-10 p-4 pt-safe bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
          <div className="flex items-center gap-3 pointer-events-auto mt-2">
            ...
            <div className="bg-white/95 backdrop-blur-md px-4 py-2.5 rounded-2xl shadow-lg flex-1 min-w-0">
              <h3 className="font-bold text-slate-800 text-sm leading-tight">Navigating to Electrical Fault</h3>
        ...
        <div className="flex-1 w-full h-full relative">
          ...
          >
            ...
            {currentLocation && <Marker position={currentLocation} icon={{ url: ZIP_ICON, scaledSize: new window.google.maps.Size(60, 60), anchor: new window.google.maps.Point(30, 30) }} zIndex={100} />}
        ...
        <div className="absolute bottom-6 left-4 right-4 z-10 pb-safe">
          <div className="bg-white rounded-2xl shadow-2xl p-5 border border-slate-100">
            <div className="flex justify-between items-end mb-4">
              ...
              <div className="bg-yellow-100 p-3 rounded-full animate-pulse"><Timer className="w-6 h-6 text-yellow-600" /></div>
            </div>
            <label className={`w-full py-4 rounded-xl shadow-lg transition-all flex flex-col items-center justify-center relative overflow-hidden cursor-pointer ${!isWithinRange ? "bg-slate-100 text-slate-400 cursor-not-allowed border-2 border-slate-200" : "bg-yellow-600 text-white shadow-yellow-500/30"}`}>
              {uploadingId === activeTask?.id ? <span className="flex items-center gap-2 font-bold animate-pulse"><Loader className="w-5 h-5 animate-spin" /> Verifying Live Circuit...</span> : !isWithinRange ? <><span className="flex items-center gap-2 font-bold text-sm"><AlertTriangle className="w-4 h-4" /> Move Closer</span><span className="text-[10px] font-bold mt-1 opacity-70">{Math.round(distanceMeters - 50)}m to fault</span></> : <span className="flex items-center gap-2 font-bold text-lg"><Camera className="w-5 h-5" /> Capture Fixed Asset</span>}
    ...
    <div className="min-h-screen bg-slate-50 pb-20 font-sans max-w-md mx-auto shadow-2xl overflow-hidden relative border-x border-slate-200">
      ...
      <div className="bg-slate-900 text-white p-6 rounded-b-[2rem] shadow-lg sticky top-0 z-10">
        <div className="flex justify-between items-start mb-6">
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Power Grid Division</p>
            <h1 className="text-2xl font-black">{user?.name || "Grid Technician"}</h1>
            <div className="flex items-center gap-2 mt-2">
              <span className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse"/>
              <span className="text-xs font-bold text-yellow-400 uppercase">Live Grid Asset Tracking</span>
            </div>
          </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="bg-slate-800/50 flex-1 p-3 rounded-xl border border-slate-700 backdrop-blur-sm">
             <span className="text-2xl font-black text-yellow-400 block">{historyCount}</span>
             <span className="text-[10px] text-slate-400 uppercase font-bold">Grid Restored</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mt-6 flex gap-6 border-b border-slate-200">
        <button onClick={() => setActiveTab("active")} className={`pb-3 text-sm font-bold relative ${activeTab === "active" ? "text-slate-900" : "text-slate-400"}`}>
          Maintenance
          {activeTab === "active" && <span className="absolute bottom-0 left-0 w-full h-1 bg-yellow-600 rounded-t-full" />}
        </button>
        <button onClick={() => setActiveTab("history")} className={`pb-3 text-sm font-bold relative ${activeTab === "history" ? "text-slate-900" : "text-slate-400"}`}>
          Repair History
          {activeTab === "history" && <span className="absolute bottom-0 left-0 w-full h-1 bg-yellow-600 rounded-t-full" />}
        </button>
      </div>
      ...
      <div className="p-6 space-y-6">
        {loading ? (
           <div className="text-center py-10 text-slate-400">
             ...
             <p className="text-sm font-bold">Scanning grid status...</p>
           </div>
        ) : displayTasks.length === 0 ? (
          <div className="text-center py-10 text-slate-400">
            <Zap className="w-12 h-12 mx-auto mb-3 opacity-20" />
            <p className="text-sm font-medium">{activeTab === 'active' ? "The grid is stable!" : "No repair history."}</p>
          </div>
        ) : (
                ...
                <div>
                  ...
                  {(activeTab === 'history' || task.status === 'RESOLVED') && task.aiData && (
                    <div className="mt-2 p-2 bg-yellow-50 rounded-lg border border-yellow-100">
                        <p className="text-[10px] text-yellow-700 font-medium"> <span className="font-bold">AI Note:</span> {task.aiData.reasoning} </p>
                    </div>
                  )}
                </div>
              </div>
              {activeTab === 'active' && (
                <button onClick={() => handleStartNavigation(task)} className="w-full flex items-center justify-center gap-2 py-3.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 shadow-lg shadow-yellow-100">
