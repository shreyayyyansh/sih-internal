...
const WATER_ICON = "https://cdn-icons-png.flaticon.com/512/3105/3105807.png"; // Water droplet or similar
const ROUTE_COLOR = "#0ea5e9"; // Blue for water

const db = getDatabase();

export default function WaterStaffDashboard() {
  ...
  useEffect(() => {
    ...
    const updateLocation = (position) => {
      ...
      const path = `staff/water/${geohash}/${sanitizedUserId}`;
      const userRef = ref(db, path);

      const staffData = {
        name: user.name || "Water Specialist",
        email: user.email,
  ...
  const handleUploadProof = async (taskId, event) => {
    ...
    try {
      ...
      setVerificationResult({
        success: true,
        title: "Repairs Verified!",
        message: reasoning || "The AI confirmed the water issue has been resolved.",
      ...
      <div className="fixed inset-0 z-[6000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
        <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
          <div className="flex justify-center mb-4">
            <div className={`p-4 rounded-full ${isSuccess ? 'bg-sky-100' : 'bg-rose-100'}`}>
              {isSuccess ? <ShieldCheck className="w-10 h-10 text-sky-600" /> : <AlertOctagon className="w-10 h-10 text-rose-600" />}
            </div>
          </div>
          <h2 className={`text-2xl font-black text-center mb-2 ${isSuccess ? 'text-sky-800' : 'text-rose-800'}`}>{verificationResult.title}</h2>
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6">
            <p className="text-slate-600 text-sm font-medium text-center leading-relaxed">"{verificationResult.message}"</p>
          </div>
          <button onClick={() => setVerificationResult(null)} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg ${isSuccess ? 'bg-sky-600 hover:bg-sky-700' : 'bg-slate-900'}`}>
      ...
      <div className="fixed inset-0 z-[5000] bg-white h-[100dvh] w-full flex flex-col relative">
        ...
        <div className="absolute top-0 left-0 right-0 z-10 p-4 pt-safe bg-gradient-to-b from-black/60 to-transparent pointer-events-none">
          <div className="flex items-center gap-3 pointer-events-auto mt-2">
            ...
            <div className="bg-white/95 backdrop-blur-md px-4 py-2.5 rounded-2xl shadow-lg flex-1 min-w-0">
              <h3 className="font-bold text-slate-800 text-sm leading-tight">Navigating to Water Issue</h3>
        ...
        <div className="flex-1 w-full h-full relative">
          ...
          >
            ...
            {currentLocation && <Marker position={currentLocation} icon={{ url: WATER_ICON, scaledSize: new window.google.maps.Size(60, 60), anchor: new window.google.maps.Point(30, 30) }} zIndex={100} />}
        ...
        <div className="absolute bottom-6 left-4 right-4 z-10 pb-safe">
          <div className="bg-white rounded-2xl shadow-2xl p-5 border border-slate-100">
            <div className="flex justify-between items-end mb-4">
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Arrival Estimate</p>
              ...
              <div className="bg-sky-100 p-3 rounded-full animate-pulse"><Timer className="w-6 h-6 text-sky-600" /></div>
            </div>
            <label className={`w-full py-4 rounded-xl shadow-lg transition-all flex flex-col items-center justify-center relative overflow-hidden cursor-pointer ${!isWithinRange ? "bg-slate-100 text-slate-400 cursor-not-allowed border-2 border-slate-200" : "bg-sky-600 text-white shadow-sky-500/30"}`}>
              {uploadingId === activeTask?.id ? <span className="flex items-center gap-2 font-bold animate-pulse"><Loader className="w-5 h-5 animate-spin" /> Verifying...</span> : !isWithinRange ? <><span className="flex items-center gap-2 font-bold text-sm"><AlertTriangle className="w-4 h-4" /> Move Closer</span><span className="text-[10px] font-bold mt-1 opacity-70">{Math.round(distanceMeters - 50)}m to target</span></> : <span className="flex items-center gap-2 font-bold text-lg"><Camera className="w-5 h-5" /> Resolve Issue</span>}
    ...
    <div className="min-h-screen bg-slate-50 pb-20 font-sans max-w-md mx-auto shadow-2xl overflow-hidden relative border-x border-slate-200">
      ...
      <div className="bg-slate-900 text-white p-6 rounded-b-[2rem] shadow-lg sticky top-0 z-10">
        <div className="flex justify-between items-start mb-6">
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Water & Sanitation</p>
            <h1 className="text-2xl font-black">{user?.name || "Water Specialist"}</h1>
            <div className="flex items-center gap-2 mt-2">
              <span className="w-2 h-2 bg-sky-400 rounded-full animate-pulse"/>
              <span className="text-xs font-bold text-sky-400 uppercase">Tracking Active</span>
            </div>
          </div>
        ...
        <div className="flex gap-4">
          <div className="bg-slate-800/50 flex-1 p-3 rounded-xl border border-slate-700 backdrop-blur-sm">
            ...
            <span className="text-[10px] text-slate-400 uppercase font-bold">Incomplete</span>
          </div>
          <div className="bg-slate-800/50 flex-1 p-3 rounded-xl border border-slate-700 backdrop-blur-sm">
             <span className="text-2xl font-black text-sky-400 block">{historyCount}</span>
             <span className="text-[10px] text-slate-400 uppercase font-bold">Repaired Today</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mt-6 flex gap-6 border-b border-slate-200">
        <button onClick={() => setActiveTab("active")} className={`pb-3 text-sm font-bold relative ${activeTab === "active" ? "text-slate-900" : "text-slate-400"}`}>
          My Assignments
          {activeTab === "active" && <span className="absolute bottom-0 left-0 w-full h-1 bg-sky-600 rounded-t-full" />}
        </button>
        <button onClick={() => setActiveTab("history")} className={`pb-3 text-sm font-bold relative ${activeTab === "history" ? "text-slate-900" : "text-slate-400"}`}>
          Repair History
          {activeTab === "history" && <span className="absolute bottom-0 left-0 w-full h-1 bg-sky-600 rounded-t-full" />}
        </button>
      </div>
      ...
      <div className="p-6 space-y-6">
        {loading ? (
           <div className="text-center py-10 text-slate-400">
             ...
             <p className="text-sm font-bold">Scanning for leaks...</p>
           </div>
        ) : displayTasks.length === 0 ? (
          <div className="text-center py-10 text-slate-400">
            <Droplets className="w-12 h-12 mx-auto mb-3 opacity-20" />
            <p className="text-sm font-medium">{activeTab === 'active' ? "Water system is healthy!" : "No history recorded."}</p>
          </div>
        ) : (
                ...
                <div>
                  ...
                  {(activeTab === 'history' || task.status === 'RESOLVED') && task.aiData && (
                    <div className="mt-2 p-2 bg-sky-50 rounded-lg border border-sky-100">
                        <p className="text-[10px] text-sky-700 font-medium"> <span className="font-bold">AI Analysis:</span> {task.aiData.reasoning} </p>
                    </div>
                  )}
                </div>
              </div>
              {activeTab === 'active' && (
                <button onClick={() => handleStartNavigation(task)} className="w-full flex items-center justify-center gap-2 py-3.5 bg-sky-900 text-white rounded-xl text-sm font-bold hover:bg-sky-800 shadow-lg shadow-sky-100">
                  <Navigation className="w-4 h-4" /> Start Repairs
                </button>
              )}
