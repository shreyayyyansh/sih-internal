...
export default function ReceiverCategories() {
    ...
    const fetchLocationAndNavigate = async (category) => {
        try {
            ...
            router.push({ pathname: '/kindshare/receiver/ngo-list', params: { category, lat, lon } });
        } catch (error) {
        ...
        <View style={{ flex: 1, backgroundColor: '#050510' }}>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
                <View style={{ marginBottom: 24 }}>
                    <Text style={{ fontSize: 22, fontWeight: '800', color: '#fff' }}>Find Items to Receive</Text>
                    <Text style={{ fontSize: 13, color: '#a1a1aa', marginTop: 4 }}>Select a category to see available NGOs.</Text>
                </View>

                {loading && (
                    <View style={{ alignItems: 'center', paddingVertical: 16 }}>
                        <ActivityIndicator color="#818cf8" />
                        <Text style={{ color: '#818cf8', marginTop: 8, fontSize: 13 }}>Locating nearby NGOs...</Text>
                    </View>
                )}
